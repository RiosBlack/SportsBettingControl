...
export async function getUserStats() {
  try {
    ...
    const [
      ...
      prisma.bet.count({ where: { userId: session.user.id, status: 'PERDIDA' } }),
      prisma.bet.count({ where: { userId: session.user.id, status: 'ANULADA' } }),
      prisma.bet.count({ where: { userId: session.user.id, status: 'PENDENTE' } }),
    ...
    const totalProfit = allBets.reduce((sum, bet) => sum + Number(bet.profit || 0), 0)
    const totalStaked = allBets.reduce((sum, bet) => sum + Number(bet.stake), 0)
    ...
    const avgOdds = wonBetsWithOdds.length > 0
      ? wonBetsWithOdds.reduce((sum, bet) => sum + Number(bet.odds), 0) / wonBetsWithOdds.length
      : 0
...
export async function getStatsBySport() {
  try {
    ...
    return { error: error.message || 'Erro ao buscar estatísticas por esporte' }
  }
}
...
export async function getMonthlyStats(year?: number) {
  try {
    ...
    const stats = monthlyData.map((data) => {
      ...
      const roi = data.totalStaked > 0 ? (data.totalProfit / data.totalStaked) * 100 : 0
